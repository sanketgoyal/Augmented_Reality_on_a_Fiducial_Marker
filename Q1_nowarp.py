    ...
    if list_for_corners != []:		#if listforcorners is not empty then go forward
        for i in range(0,len(list_for_corners)):
            ...
            if 0 not in corner_position:  
                ...
                l,m,n = TagId(im_out)
                cv2.putText(image, str(n), ((corner_position[0][0]-50, corner_position[0][1]-50)), cv2.FONT_ITALIC, 2, (255,0,255),3)
                cv2.imshow('Display', image)   #Displaying the image output in a window named "Display"
            corner_position = []   
